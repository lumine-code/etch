...
describe('regressions', () => {
  it('unregisters the attached bound listener after the listener was stable across an update', () => {
    ...
    const calls = [];
    function listenerA () { calls.push(['a', this]); }
    function listenerB () { calls.push(['b', this]); }
  ...
  it('does not reinsert a removed DOM node when a keyed child changes tag', () => {
    const oldVirtualNode = dom("div", null,
      dom("span", { key: 'a' }, "a"),
      dom("span", { key: 'b' }, "b")
    );
    ...
    const newVirtualNode = dom("div", null,
      dom("div", { key: 'b' }, "b!"),
      dom("span", { key: 'c' }, "c")
    );
    patch(oldVirtualNode, newVirtualNode);

    assert.strictEqual(element.outerHTML, render(dom("div", null,
      dom("div", { key: 'b' }, "b!"),
      dom("span", { key: 'c' }, "c")
    )).outerHTML);
  ...
  it('keeps scheduling updates after an update request throws', () => {
    ...
    try {
      scheduler.updateDocument(() => { throw new Error('boom'); });
  ...
  it('stays in asynchronous update mode after updateSync throws on a root node type change', async () => {
    ...
    const component = {
      ...
      update () {},
      writeAfterUpdate () { events.push('write'); },
      readAfterUpdate () { events.push('read'); }
    };
