...
function updateProps (domNode, oldVirtualNode, oldProps, newVirtualNode, newProps) {
  if (oldProps) {
    for (var name in oldProps) {
      ...
      // `class` is normalized to `className` by `dom`; assigning it on an HTML
      // element would only create a useless expando property. SVG elements
      // still need it, as their class is applied via setAttribute.
      if (name === 'class' && oldVirtualNode && !SVG_TAGS.has(oldVirtualNode.tag)) continue
  ...
  if (newProps) {
    for (var name in newProps) {
      if (name === 'ref' || name === 'on') continue
      if (name in EVENT_LISTENER_PROPS) continue
      if (name === 'class' && newVirtualNode && !SVG_TAGS.has(newVirtualNode.tag)) continue
      ...
      if (name === 'dataset') {
        ...
        // Assigning null or undefined to a DOM property stringifies it (e.g.
        // className would become "null"); treat it as a removal instead.
      ...
      } else if (name === 'value' && newVirtualNode
        && (newVirtualNode.tag === 'input' || newVirtualNode.tag === 'select' || newVirtualNode.tag === 'textarea')) {
        // Compare against the live DOM rather than the previous virtual node:
        // user interaction changes the value without a prop change, and a
        // controlled component must be able to revert it. Skipping the write
        // when the DOM already matches preserves the cursor position, which
        // every assignment resets.
...
function updateEventListeners (domNode, oldVirtualNode, newVirtualNode, listenerContext) {
  ...
  for (const eventName in newListeners) {
    ...
    if (newListener === oldListener) {
      // The listener attached to the DOM node may be a bound copy of an
      // earlier listener. Carry it forward on the new virtual node so a later
      // replacement or removal can unregister the function actually attached.
      if (oldVirtualNode && oldVirtualNode.boundListeners && oldVirtualNode.boundListeners[eventName]) {
        if (!newVirtualNode.boundListeners) newVirtualNode.boundListeners = {}
        newVirtualNode.boundListeners[eventName] = oldVirtualNode.boundListeners[eventName]
      }
    } else {
      if (oldListener) {
        let listenerToRemove
