...
// Marker tag for `<>…</>`. A fragment has no element of its own, so `dom`
// returns its children as a plain array and the enclosing node splices them in
// the same way it does any array child.
const Fragment = Symbol('etch.Fragment')

function dom (tag, props, ...children) {
  const ambiguous = []

  if (tag === Fragment) {
    // The enclosing node re-collects ambiguity from each spliced child, so the
    // list gathered here is discarded.
    const flattened = []
    appendChildren(flattened, children, ambiguous)
    return flattened
  }
