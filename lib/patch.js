...
// A component may update itself between two patches of the tree that contains
// it — a scheduled self-update, or an update driven by another copy of etch
// across a package boundary — and such an update can swap the component's root
// element. The enclosing tree's `domNode` then points at a node that is no
// longer in the DOM, so every read of an old virtual node's DOM node resolves
// through the component, which always knows its current element.
function currentDomNode(virtualNode) {
  const component = virtualNode.component;
  return component ? component.element : virtualNode.domNode;
}

function patch(oldVirtualNode, newVirtualNode, options) {
  const oldNode = currentDomNode(oldVirtualNode);
...
function updateChildren(parentElement, oldChildren, newChildren, options) {
  ...
  while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
    if (!oldStartChild) {
      ...
      // Use the node returned by patch: a child component may have swapped its
      // root element, leaving the old virtual node's domNode stale. Reference
      // nodes resolve through currentDomNode for the same reason.
      parentElement.insertBefore(
        patch(oldStartChild, newEndChild, options),
        currentDomNode(oldEndChild).nextSibling,
      );
      oldStartChild = oldChildren[++oldStartIndex];
      newEndChild = newChildren[--newEndIndex];
      ...
      if (oldIndex == null) {
        parentElement.insertBefore(render(newStartChild, options), currentDomNode(oldStartChild));
        newStartChild = newChildren[++newStartIndex];
      } else {
        var oldChildToMove = oldChildren[oldIndex];
        // Use the node returned by patch: when the matched child has the same
        // key but a different tag (or a component swapped its root element),
        // patch replaces the DOM node and the old virtual node's domNode is a
        // stale reference to the removed node.
        var nodeToMove = patch(oldChildToMove, newStartChild, options);
        if (oldIndex === oldStartIndex) {
          // The matched child is the start child itself, with a different tag:
          // patch already replaced it in place, and moving it before the node
          // it just displaced would use a removed node as the reference.
          // Advancing retires the old virtual node from the diff instead.
          oldStartChild = oldChildren[++oldStartIndex];
        } else {
          oldChildren[oldIndex] = undefined;
          parentElement.insertBefore(nodeToMove, currentDomNode(oldStartChild));
        }
        newStartChild = newChildren[++newStartIndex];
      }
    }
  ...
  if (oldStartIndex > oldEndIndex) {
    var subsequentElement = newChildren[newEndIndex + 1]
      ? newChildren[newEndIndex + 1].domNode
      : null;
...
function removeVirtualNode(virtualNode, refs, removeDOMNode = true) {
  const { props, children, component } = virtualNode;
  const domNode = currentDomNode(virtualNode);
...
function virtualNodesAreEqual(oldVirtualNode, newVirtualNode) {
  return (
    getKey(oldVirtualNode) === getKey(newVirtualNode) && oldVirtualNode.tag === newVirtualNode.tag
  );
}
