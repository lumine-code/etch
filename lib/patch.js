...
function updateChildren(parentElement, oldChildren, newChildren, options) {
  ...
  while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
    if (!oldStartChild) {
      ...
      // root element, leaving the old virtual node's domNode stale.
      parentElement.insertBefore(
        patch(oldStartChild, newEndChild, options),
        oldEndChild.domNode.nextSibling,
      );
      oldStartChild = oldChildren[++oldStartIndex];
      newEndChild = newChildren[--newEndIndex];
    } else if (virtualNodesAreEqual(oldEndChild, newStartChild)) {
      parentElement.insertBefore(patch(oldEndChild, newStartChild, options), oldStartChild.domNode);
      oldEndChild = oldChildren[--oldEndIndex];
      newStartChild = newChildren[++newStartIndex];
    } else {
      if (!oldIndicesByKey) {
      ...
      if (oldIndex == null) {
        ...
        parentElement.insertBefore(nodeToMove, oldStartChild.domNode);
        newStartChild = newChildren[++newStartIndex];
      }
    }
  }
