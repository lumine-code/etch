...
const componentsBeingUpdatedSync = new Set();
const componentsNeedingFollowUpUpdate = new Set();
...
// A fragment renders as an array of siblings, which has no single element to
// stand in for the component. Say so rather than failing later in `render`.
function assertNotFragment(component, virtualNode) {
  if (!Array.isArray(virtualNode)) return;
  const namePart =
    component.constructor && component.constructor.name ? " in " + component.constructor.name : "";
  throw new Error("render() must return a single element, not a fragment" + namePart);
}

function applyContext(context, virtualNode) {
  virtualNode.context = context;
  const ambiguous = virtualNode.ambiguous;
  if (ambiguous) {
    for (let i = 0; i < ambiguous.length; i++) {
      ambiguous[i].context = context;
    }
    // Null rather than delete: deleting would push the virtual node into
    // dictionary mode and defeat hidden-class stability.
    virtualNode.ambiguous = null;
  }
}
...
function updateSync(component, replaceNode = true) {
  if (!isValidVirtualNode(component.virtualNode)) {
    throw new Error(
      `${component.constructor ? component.constructor.name + " instance" : component} is not associated with a valid virtualNode. Perhaps this component was never initialized?`,
    );
  }

  if (component.element == null) {
    throw new Error(
      `${component.constructor ? component.constructor.name + " instance" : component} is not associated with a DOM element. Perhaps this component was never initialized?`,
    );
  }

  // A component can be asked to update while its own update is still patching:
  // the patch reaches a child component, the child's `update(props)` has an
  // observable side effect (a text editor's `setText` firing `onDidChange`),
  // and the handler synchronously requests another update of the parent.
  // Diffing `component.virtualNode` again at that point patches the DOM twice
  // from the same old tree and corrupts both — so a reentrant request is
  // deferred and performed once after the in-flight pass has finished.
  if (componentsBeingUpdatedSync.has(component)) {
    componentsNeedingFollowUpUpdate.add(component);
    return;
  }

  componentsBeingUpdatedSync.add(component);
  try {
    ...
    if (!isValidVirtualNode(newVirtualNode)) {
      const namePart =
        component.constructor && component.constructor.name
          ...
          : "";
      throw new Error(
        "invalid falsy value " + newVirtualNode + " returned from render()" + namePart,
      );
    }
    assertNotFragment(component, newVirtualNode);
    ...
    try {
      ...
      if (newDomNode !== oldDomNode && !replaceNode) {
        throw new Error(
          "The root node type changed on update, but the update was performed with the replaceNode option set to false",
        );
      } else {
    ...
    componentsBeingUpdatedSync.delete(component);
  }

  // Render once more with whatever state the deferred request was about. This
  // loops until a pass completes without a new request, which any render that
  // is a pure function of current state does on its second attempt.
  if (componentsNeedingFollowUpUpdate.delete(component)) {
    updateSync(component, replaceNode);
  }
}
